...
gulp.task('serve', ['nodemon', 'inject', 'sass'], function() {
  //startBackend(function(err) {
    //if (err) {
...
gulp.task('heroku:candidate', [], function () {
  gulp.run('build');
});
...
function startBrowserSync(opt) {
  ...
  function serveApp() {
    ...
    options.proxy = "http://localhost:5000";
    //options.server = {
      //baseDir: [
      //config.client,
      //config.tmp
...
function startBackend (cb) {
  exec('nodemon --debug server.js', function(err, stdout, stderr) {
    console.log(stdout);
    console.log(stderr);
    cb(err);
  });
